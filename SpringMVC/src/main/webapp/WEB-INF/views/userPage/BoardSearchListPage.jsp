...
<body>
	<div class="page-wrapper">
		...
		%>
		<%
			Integer searchPageIdxObj = (Integer) request.getAttribute("searchPageIdx");
		...
		<div class="main_content_container" align="center">
				...
				<div class="boardContainer">
					<div class="boardBottomBox">
						<div class="boardSearchBox">
							<form action="BoardSearchPage.do" method="get"
								...
								<input type="text" class="boardSearchInput" name="searchTitle"
									...
									class="boardSearchButton" value="검색"  maxlength="10">
							</form>
							<button class="boardSearchButton" onclick="location.href='BoardListPage.do'">전체조회</button>
						</div>
						<div class="boardInsertBox">
							<button onclick="location.href='BoardInsertPage.do'"
								class="boardInsertButton">민원접수</button>
						</div>
					</div>
				</div>
			<div class="boardListContainer">
				...
				<div class="boardListTableRow">
					...
					<div class="boardListTableCell">
						<span class="link-text"
							onclick="window.location.href='BoardContentPage.do?cpl_idx=<%=complain.getCpl_idx()%>';"> 
							<%=complain.getTitle()%>
						</span>
					</div>
				...
				<% 
				Object searchTotalPagesObj = request.getAttribute("searchTotalPages");
				...
				<div class="pageMoveBox">
					...
					<%-- 첫 페이지 버튼 --%>
					<a href="BoardSearchPage.do?searchPageIdx=1&searchTitle=<%=searchTitle%>"
						class="pageMoveButton <%=(searchPageIdx > 1 ? "" : "invisible")%>">&lt;&lt;</a>

					<%-- 이전 페이지 버튼 --%>
					<a href="BoardSearchPage.do?searchPageIdx=<%=prevPage%>&searchTitle=<%=searchTitle%>"
						class="pageMoveButton <%=(searchPageIdx > 1 ? "" : "invisible")%>">&lt;</a>
					...
					for (int i = Math.max(1, searchPageIdx - 2); i <= Math.min(searchTotalPages, searchPageIdx + 2); i++) {
						...
						if (i == searchPageIdx) { %>
							...
							<a href="BoardSearchPage.do?searchPageIdx=<%=i%>&searchTitle=<%=searchTitle%>" class="pageMoveButton"><%=i%></a>
							<%
						}%><%
					}%>

					<%-- 다음 페이지 버튼 --%>
					<a href="BoardSearchPage.do?searchPageIdx=<%=nextPage%>&searchTitle=<%=searchTitle%>"
						class="pageMoveButton <%=(searchPageIdx < searchTotalPages ? "" : "invisible")%>">&gt;</a>

					<%-- 마지막 페이지 버튼 --%>
					<a href="BoardSearchPage.do?searchPageIdx=<%=searchTotalPages%>&searchTitle=<%=searchTitle%>"
						class="pageMoveButton <%=(searchPageIdx < searchTotalPages ? "" : "invisible")%>">&gt;&gt;</a>
