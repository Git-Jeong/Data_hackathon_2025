...
<body>
	<div class="page-wrapper">
		...
		<div class="main_content_container" align="center">
			...
			<h3 class="text-center my-4">민원 목록</h3>
						...
						<div class="boardSearchBox">
							<form action="BoardSearchPage.do" method="get"
								...
								<input type="text" class="boardSearchInput" name="searchTitle"
									...
									type="submit" class="boardSearchButton" value="검색" maxlength="10">
							</form>
							<button class="boardSearchButton" style="visibility: hidden;" onclick="location.href='BoardListPage.do'">전체조회</button>
						</div> <!-- boardSearchBox종 -->
			...
			<div class="boardListContainer">
				...
				<%
				Object pageIdxObj = request.getAttribute("pageIdx");
				...
				<div class="pageMoveBox">
					...
					<%-- 첫 페이지 버튼 --%>
					<a href="BoardListPage.do?pageIdx=1"
						class="pageMoveButton <%=(pageIdx > 1 ? "" : "invisible")%>">&lt;&lt;</a>

					<%-- 이전 페이지 버튼 --%>
					<a href="BoardListPage.do?pageIdx=<%=prevPage%>"
						class="pageMoveButton <%=(pageIdx > 1 ? "" : "invisible")%>">&lt;</a>
					...
					for (int i = Math.max(1, pageIdx - 2); i <= Math.min(totalPages, pageIdx + 2); i++) {
						...
						if (i == pageIdx) {
							...
							<a href="BoardListPage.do?pageIdx=<%=i%>" class="pageMoveButton"><%=i%></a>
							<%
						}%>
						<%
					}%>
					...
					<%-- 다음 페이지 버튼 --%>
					<a href="BoardListPage.do?pageIdx=<%=nextPage%>"
						class="pageMoveButton <%=(pageIdx < totalPages ? "" : "invisible")%>">&gt;</a>

					<%-- 마지막 페이지 버튼 --%>
					<a href="BoardListPage.do?pageIdx=<%=totalPages%>"
						class="pageMoveButton <%=(pageIdx < totalPages ? "" : "invisible")%>">&gt;&gt;</a>
